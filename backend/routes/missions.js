...
import { missionService as missionServiceSingleton } from '../services/mission.service.js';
...
router.get('/', 
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Get missions + user's mission progress
      const { missions, userProgress } = await missionServiceSingleton.listMissions(filters, userId);
...
router.get('/:missionId', 
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const { missions } = await missionServiceSingleton.listMissions({}, userId);
      const mission = missions.find(m => m.id === missionId) || null;
      if (!mission) {
        return res.status(404).json({ success: false, message: 'Mission not found' });
      }

      const { userProgress } = await missionServiceSingleton.listMissions({}, userId);
...
router.post('/start', 
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await missionServiceSingleton.startMission(userId, missionId);
    if (!result.ok) {
...
router.post('/complete', 
  ...
  asyncHandler(async (req, res) => {
    ...
    const result = await missionServiceSingleton.completeMission(userId, missionId, completionData);
    if (!result.ok) {
      return res.status(result.status).json({ success: false, message: result.message });
    }
...
router.post('/join', 
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      await missionServiceSingleton.startMission(userId, missionId);
...
router.get('/user/active', 
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      const { userProgress, missions } = await missionServiceSingleton.listMissions({}, userId);
...
/** @param {{ missionService: import('../services/mission.service.js').MissionService }} deps */
