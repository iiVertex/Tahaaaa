...
CREATE TABLE users (
    ...
    clerk_id VARCHAR(255) UNIQUE, -- Clerk authentication user ID (nullable for backward compatibility)
    ...
    coins INTEGER DEFAULT 1000 CHECK (coins >= 0), -- New users start with 1000 coins
...
CREATE TABLE missions (
    ...
    badge VARCHAR(50), -- badge icon for missions (falcon, date_palm, family, etc.)
...
-- Mission Steps Table - AI-generated 3-step execution plan for each started mission
-- When a mission is started, AI generates exactly 3 actionable steps stored here
CREATE TABLE mission_steps (
    ...
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    UNIQUE(user_mission_id, step_number)
);

CREATE INDEX idx_mission_steps_user_mission ON mission_steps(user_mission_id);
...
-- Quote Sessions - Persist user quote flows
...
-- User Purchases - Track actual product purchases for multi-product customer analysis
CREATE TABLE user_purchases (
    ...
    status VARCHAR(20) DEFAULT 'active' CHECK (status IN ('active', 'cancelled', 'expired', 'renewed')),
    metadata JSONB DEFAULT '{}', -- additional purchase details
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE TABLE rewards (
    ...
    xp_reward INTEGER DEFAULT 0 CHECK (xp_reward >= 0), -- XP reward when redeeming this reward
    badge_icon VARCHAR(100),
    badge_rarity VARCHAR(20) DEFAULT 'common' CHECK (badge_rarity IN ('common', 'rare', 'epic', 'legendary')),
    partner_offer JSONB DEFAULT '{}',
    coupon_code VARCHAR(50), -- Coupon code for redeemed rewards
    is_active BOOLEAN DEFAULT TRUE,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- NOTE: All profile fields (name, age, gender, nationality, budget, vulnerabilities, insurance_preferences, areas_of_interest, first_time_buyer)
-- MUST persist per account when Clerk authentication layer is integrated. These fields are stored in profile_json JSONB field.
CREATE TABLE user_profiles (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    profile_json JSONB NOT NULL DEFAULT '{}', -- Stores: name, age (18-100), gender, nationality, budget (0-30000 QAR), vulnerabilities, insurance_preferences, areas_of_interest, first_time_buyer
    encrypted_data TEXT,
...
CREATE INDEX idx_users_clerk_id ON users(clerk_id) WHERE clerk_id IS NOT NULL; -- Clerk user lookup
...
-- Quote session indexes
CREATE INDEX idx_user_quotes_user_status ON user_quotes(user_id, status);
CREATE INDEX idx_user_quotes_created ON user_quotes(created_at DESC);

-- Purchase tracking indexes
CREATE INDEX idx_user_purchases_user_date ON user_purchases(user_id, purchase_date DESC);
CREATE INDEX idx_user_purchases_type ON user_purchases(product_type);
CREATE INDEX idx_user_purchases_status ON user_purchases(status);

-- Enable RLS and policies for user_quotes
ALTER TABLE user_quotes ENABLE ROW LEVEL SECURITY;
CREATE POLICY user_quotes_select_own ON user_quotes
    FOR SELECT USING (COALESCE(current_user_uuid(), auth.uid()) = user_id);
CREATE POLICY user_quotes_insert_own ON user_quotes
    FOR INSERT WITH CHECK (COALESCE(current_user_uuid(), auth.uid()) = user_id);
CREATE POLICY user_quotes_update_own ON user_quotes
...
-- Play Activity Table - Track Road-Trip Roulette spins and other play features
CREATE TABLE play_activity (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    user_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    activity_type VARCHAR(50) NOT NULL CHECK (activity_type IN ('roulette_spin', 'mini_game', 'challenge')),
    activity_data JSONB DEFAULT '{}', -- Store roulette spin results, itinerary, rewards, etc.
    coins_earned INTEGER DEFAULT 0 CHECK (coins_earned >= 0),
    xp_earned INTEGER DEFAULT 0 CHECK (xp_earned >= 0),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    activity_date DATE DEFAULT CURRENT_DATE -- Track daily limits (max 3 spins/day)
);
CREATE INDEX idx_play_activity_user_date ON play_activity(user_id, activity_date);
CREATE INDEX idx_play_activity_type ON play_activity(activity_type);
ALTER TABLE play_activity ENABLE ROW LEVEL SECURITY;
CREATE POLICY play_activity_select_own ON play_activity
    FOR SELECT USING (COALESCE(current_user_uuid(), auth.uid()) = user_id);
CREATE POLICY play_activity_insert_own ON play_activity
    FOR INSERT WITH CHECK (COALESCE(current_user_uuid(), auth.uid()) = user_id);

-- Referrals tracking (for share/click/install funnels)
...
CREATE INDEX idx_user_referrals_owner ON user_referrals(owner_user_id);
CREATE INDEX idx_user_referrals_created ON user_referrals(created_at DESC);
ALTER TABLE user_referrals ENABLE ROW LEVEL SECURITY;
CREATE POLICY user_referrals_select_own ON user_referrals
    FOR SELECT USING (COALESCE(current_user_uuid(), auth.uid()) = owner_user_id);
CREATE POLICY user_referrals_insert_own ON user_referrals
    FOR INSERT WITH CHECK (COALESCE(current_user_uuid(), auth.uid()) = owner_user_id);
CREATE POLICY user_referrals_update_own ON user_referrals
    FOR UPDATE USING (COALESCE(current_user_uuid(), auth.uid()) = owner_user_id);
...
-- Function to get current user UUID from JWT claims (for Clerk compatibility)
-- Backend sets user_id in JWT claims after Clerk token verification
-- Falls back to auth.uid() if Supabase Auth is used
...
-- Updated to use current_user_uuid() for Clerk compatibility
-- Falls back to auth.uid() if Supabase Auth is used
CREATE POLICY users_select_own ON users
    FOR SELECT USING (COALESCE(current_user_uuid(), auth.uid()) = id);
    
CREATE POLICY users_update_own ON users
    FOR UPDATE USING (COALESCE(current_user_uuid(), auth.uid()) = id);
...
CREATE POLICY social_connections_select_own ON social_connections
    FOR SELECT USING (COALESCE(current_user_uuid(), auth.uid()) = user_id OR COALESCE(current_user_uuid(), auth.uid()) = friend_id);
...
CREATE POLICY social_connections_update_own ON social_connections
    FOR UPDATE USING (COALESCE(current_user_uuid(), auth.uid()) = user_id OR COALESCE(current_user_uuid(), auth.uid()) = friend_id);
...
COMMENT ON TABLE play_activity IS 'Road-Trip Roulette spins and play feature activity tracking (max 3 spins/day per user)';
COMMENT ON TABLE user_quotes IS 'User quote sessions for insurance products';
COMMENT ON TABLE user_purchases IS 'Track actual product purchases for multi-product customer analysis';
...
-- Total Tables Created: 21 (users, user_behavior_events, lifescore_history, ai_recommendations, product_recommendations, missions, mission_instances, user_missions, mission_steps, achievements, user_achievements, notifications, user_sessions, rewards, user_rewards, scenarios, user_scenarios, social_connections, user_profiles, onboarding_responses, play_activity, user_quotes, user_purchases, user_referrals)
-- New Tables: 12 (user_behavior_events, lifescore_history, ai_recommendations, product_recommendations, mission_instances, achievements, user_achievements, notifications, user_sessions, play_activity, mission_steps)
-- Enhanced Tables: 3 (users with coins DEFAULT 1000, missions with badge column, user_profiles)
-- Existing Tables: 9 (rewards with coupon_code, user_rewards, scenarios, user_scenarios, social_connections, onboarding_responses, user_quotes, user_purchases, user_referrals)

-- RLS Policies: 21+ tables with comprehensive security (all user-facing tables)
